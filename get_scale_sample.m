function scale_sample = get_scale_sample(fea, scaleFactors, scale_window)
%% Todo: weight each scale_sample with a hann window
...
% for scale_id = 1:length(scaleFactors)
%     re_sz_float = floor(fea_sz(1:2)*scaleFactors(scale_id));
%     if mod(re_sz_float(1), 2) == 0
%         re_sz = re_sz_float;
%         fea_resized = imresize(fea, re_sz);
%     else
%         re_sz = re_sz_float + 1;
%         fea_resized = imresize(imresize(fea, re_sz_float), re_sz);
%     end
%     xs_start = max((re_sz(2) - fea_sz(2))/2, 0) + 1;
%     ys_start = max((re_sz(1) - fea_sz(1))/2, 0) + 1;
%     xs_end = re_sz(2) - xs_start + 1;
