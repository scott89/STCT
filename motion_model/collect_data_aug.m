...
seq = dir(seq_path);
train_path = 'data_60/train/';
val_path = 'data_60/val/';
if ~isdir(train_path)
    mkdir(train_path);
end
if ~isdir(val_path)
...
%     location = location(end:-1:1, :);
    ...
    for scale_id = -2:2
        scale = 1.2^(scale_id);
        for alpha = alpha_step:alpha_step:2 * pi
            location_off = [location_off; off * (rotate_mat(alpha))' * scale];
            location_off_cell = {location_off_cell{:}, off * (rotate_mat(alpha))' * scale};
        end
    end
...
for seq_id = 1:length(location_off_cell)
    samp_num = 1;
    rand_key = rand(1);
    fprintf('Seq: %d / %d \n', seq_id, length(location_off_cell));
    for step = 1:20:size(location_off_cell{seq_id}, 1) - time_step
